...
# ────────────────────────────────────────────────
# Variáveis para o sistema de Área de Dano AOE com posicionamento pelo mouse
@export var cena_area_dano_aoe: PackedScene # Cena da área real (com dano) - arraste ÁreaDanoAOE.tscn
@export var cena_pre_visualizacao_dano_aoe: PackedScene # Cena da pré-visualização (apenas visual) - arraste PréVisualizaçãoDanoAOE.tscn
var modo_posicionamento_dano_aoe: bool = false
var pre_visualizacao_dano_instancia: Node3D = null
# ────────────────────────────────────────────────
func _ready() -> void:
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		elif modo_posicionamento_dano_aoe:
			_colocar_area_dano_aoe()
		else:
			_selecionar_npc()
	...
	if event is InputEventKey and event.pressed:
		match event.keycode:
			...
			KEY_3:
				_alternar_modo_posicionamento_dano_aoe()
func _process(_delta: float) -> void:
	...
	# Atualiza a posição da pré-visualização de dano enquanto o modo está ativo
	if modo_posicionamento_dano_aoe and pre_visualizacao_dano_instancia:
		var posicao_mouse = get_viewport().get_mouse_position()
		var origem_ray = camera.project_ray_origin(posicao_mouse)
		var fim_ray = origem_ray + camera.project_ray_normal(posicao_mouse) * 1000.0
		var consulta_ray = PhysicsRayQueryParameters3D.create(origem_ray, fim_ray)
		...
		var resultado = estado_espaco.intersect_ray(consulta_ray)
		if resultado and resultado.has("position"):
			pre_visualizacao_dano_instancia.global_position = resultado.position + Vector3(0, 0.1, 0)
			pre_visualizacao_dano_instancia.visible = true
		else:
			pre_visualizacao_dano_instancia.visible = false
...
func _curar_alvo() -> void:
	if npc_selecionado:
		...
		if hp:
			...
			npc_selecionado.get_node('Particulas_de_cura').set_emitting(true)
...
func _colocar_area_cura_aoe() -> void:
	...
	_alternar_modo_posicionamento_aoe()
# ────────────────────────────────────────────────
# Sistema de Área de Dano AOE com Posicionamento
# ────────────────────────────────────────────────
func _alternar_modo_posicionamento_dano_aoe() -> void:
	modo_posicionamento_dano_aoe = not modo_posicionamento_dano_aoe
	if modo_posicionamento_dano_aoe:
		if not cena_pre_visualizacao_dano_aoe:
			print("Cena de pré-visualização de dano AOE não configurada!")
			modo_posicionamento_dano_aoe = false
			return
		
		pre_visualizacao_dano_instancia = cena_pre_visualizacao_dano_aoe.instantiate()
		get_tree().current_scene.add_child(pre_visualizacao_dano_instancia)
		pre_visualizacao_dano_instancia.visible = true
		print("Modo de posicionamento DANO AOE ativado! Mire e clique para confirmar.")
	else:
		if pre_visualizacao_dano_instancia:
			pre_visualizacao_dano_instancia.queue_free()
			pre_visualizacao_dano_instancia = null
		print("Modo de posicionamento DANO AOE desativado.")
func _colocar_area_dano_aoe() -> void:
	if not cena_area_dano_aoe or not pre_visualizacao_dano_instancia:
		print("Não é possível colocar área de dano AOE!")
		return
	
	var instancia_area = cena_area_dano_aoe.instantiate()
	get_tree().current_scene.add_child(instancia_area)
	instancia_area.global_position = pre_visualizacao_dano_instancia.global_position
	
	print("Área de DANO AOE colocada com sucesso!")
